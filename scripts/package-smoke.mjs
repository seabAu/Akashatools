...
assert.equal(toEpochMilliseconds({ seconds: 1, nanoseconds: 500_000_000 }), 1500);
assert.equal(toEpochMilliseconds({ seconds: 1, nanos: 250_000_000 }), 1250);
assert.equal(granularToEpochMilliseconds, toEpochMilliseconds);
assert.equal(toDate({ seconds: 1 })?.toISOString(), "1970-01-01T00:00:01.000Z");
...
const timestamp: TimestampInput = { seconds: 1, nanoseconds: 500_000_000 };
...
const invalidTimestamp: TimestampInput = { seconds: 1, nanoseconds: 1, nanos: 1 };
const milliseconds: number | null = toEpochMilliseconds(timestamp);
const granularMilliseconds: number | null = granularToEpochMilliseconds(timestamp);
const instant: Date | null = toDate(timestamp);
...
void [chunks, granularChunks, nested, primaryType, timestamp, protobufTimestamp, invalidTimestamp, milliseconds, granularMilliseconds, instant, control, dataType, inputType, controlType, hasId, granularHasId, response, contract, code, checksum, digest, archivePath, archivePaths, initialize, cachedLength, length, debouncedLength, pendingLength, throttledLength, throttledPending];
`,
  );
  await writeFile(
