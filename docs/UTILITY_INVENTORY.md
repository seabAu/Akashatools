...
| `has` | Recursively checks for a key, mixing arrays/objects and legacy validity rules. | Use `hasAtPath` for known paths or `hasDeep`/`deepQuery(value).has` for bounded recursive key/value discovery. | Current consumer audit and 2.x path/query tests. |
| `hasAll` | Attempts recursive presence of all keys, with loop-return control-flow defects. | Reject implementation; compose the intended every-key policy explicitly with bounded `hasDeep` calls. | Defect proven; current consumer audit and 2.x query tests. |
...
| `objContains` | Recursively searches object values but relies on returns inside `forEach`. | Adopted exact-match replacement `hasDeep`; predicate search uses `findDeep`/`findAllDeep`. | Source reviewed; 2.x query tests. |
| `arrayContains` | Recursively searches array values with inconsistent object handling. | Adopted deep replacement `hasDeep`; primitive membership still uses native `includes`. | Source reviewed; 2.x query tests. |
| `deepGetKey` | Recursively returns values associated with a matching key. | Adopted explicit projections `findDeepValue`/`findAllDeepValues` with `{ by: "key" }`. | Current consumer audit and 2.x bounded query tests. |
| `deepSearch` | Recursively searches a named key using a predicate and optionally returns a parent. | Split into `findDeepMatch`, `findDeepValue`, and `findDeepParent`; predicate matching retains `findDeep`. | Current consumer audit and 2.x bounded query tests. |
| `deepSearchItems` | Deep search variant that calls `this.deepSearchItems`, making module invocation fragile. | Reject implementation; adopted all-match replacement `findAllDeepMatches`. | Defect proven from source; 2.x query tests. |
...
| `getFieldType` | Maps runtime values to HTML/form control concepts. | Split into `input.inputTypeForValue` and full-array `input.controlTypeForValue`; component/layout/schema policy stays app-local. | Active-call-site regression review and 2.x input tests. |
