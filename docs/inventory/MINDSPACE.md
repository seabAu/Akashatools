...
| `arrayToEnum` | Duplicate frozen value-to-itself object builder with unsafe key ambiguity. | Replaced by `array.keyBy`; the canonical `Map` index preserves key identity and makes first/last/error duplicate behavior explicit. |
...
| Mindspace export | Finding | Akashatools disposition |
| --- | --- | --- |
| `getDefaultValueForType` | Recursively constructs defaults for Mindspace custom type constructors. | Compose `data.initializeLike`/`defaultValueForType` with caller factories; custom fixture policy remains app-local. |
...
- [x] Browser/environment client candidates: all 77 exports plus four
  initially zero-declaration/side-effect candidates dispositioned in
