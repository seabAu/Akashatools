...
| Portfolio response filenames and legacy filesystem helpers accepted path components, unchecked traversal, or called an undeclared glob. | Bounded filename parsing, Node-only lexical/realpath containment, and deterministic native `globPaths`. | Source-regression boundary case plus HTTP and Node threat/discovery tests. |
