...
| Mindspace independently coerced Date values, integer timestamp strings, and Firestore `{seconds, nanoseconds}` records in notifications, reminders, task deferral, and its messaging worker; some variants invoked arbitrary `toDate` methods or silently accepted malformed fields. | `toEpochMilliseconds` is the strict atomic normalizer and `toDate` is its Date-returning wrapper. Numeric-string units are explicit; Firestore `nanoseconds` and Protobuf-message `nanos` records use exclusive, own-data fields with standard ranges; accessors/coercion methods are not invoked. | Date adversarial tests, Mindspace compatibility fixture, and `SOURCE_REFRESH_2026-07-19.md`. |
...
| A member-name inventory could not reveal discarded mutation results, ignored legacy arguments, swallowed-error dependence, dynamic access, or truthy function references. | The read-only consumer scanner now classifies call/reference shape, arguments, result context, `try` containment, direct/unused bindings, dynamic roots, and malformed-source candidates. | `test/legacy-consumer-audit.test.js` and `docs/inventory/CONSUMER_BEHAVIOR_AUDIT_2026-07-18.md`. |
