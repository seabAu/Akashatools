...
| Excepted Geo helpers reversed GeoJSON order, randomized coordinates, and ignored nearby thresholds; Schema input formatting lost decimals and guessed event/date behavior; DOM/LocalDB modules touched ambient globals and product state. | Canonical coordinate normalization and Haversine units, strict one-shot/compiled datatype parsing, late-bound media/control adapters, and explicit strict-JSON Storage calls. | Named excepted-module source regression plus geo, input, browser, benchmark, and Playwright contracts. |
